...
interface Update<T>{
    findupdateOne(id:number, input:T): Promise<T>
}
...
interface StudentRepository extends FindMany<Etudiant>, FindOne<Etudiant>, Create<Etudiant>, Update<Etudiant>, Delete{
    
}


interface CourseRepository extends FindMany<Cours>, FindOne<Cours>, Create<Cours>, Update<Cours>, Delete{
...
let liste_etudiants:Etudiant[] = [];

class SQLStudentRepository implements StudentRepository{
    async findMany(): Promise<Etudiant[]> {
    ...
    async create(input: Etudiant): Promise<Etudiant> {
        return etudiant;
    }
    ...
    async findupdateOne(id: number, input: Etudiant): Promise<Etudiant> {
        return etudiant;
    }
}
